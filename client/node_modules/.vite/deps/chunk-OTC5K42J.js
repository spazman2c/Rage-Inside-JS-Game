...
var Bone = class _Bone extends Node {
   ...
   */
  constructor(name6, skeleton, parentBone = null, localMatrix = null, restMatrix = null, bindMatrix = null, index = null) {
    super(name6, skeleton.getScene());
    this.name = name6;
    this.children = [];
    this.animations = [];
    this._index = null;
...
Mesh._instancedMeshFactory = (name6, mesh) => {
  const instance = new InstancedMesh(name6, mesh);
...
var InstancedMesh = class extends AbstractMesh {
   ...
   */
  constructor(name6, source) {
    super(name6, source.getScene());
   ...
   */
  createInstance(name6) {
    return this._sourceMesh.createInstance(name6);
  }
  /**
   ...
   * @returns the clone
   */
  clone(name6, newParent = null, doNotCloneChildren, newSourceMesh) {
    const result = (newSourceMesh || this._sourceMesh).createInstance(name6);
...
var AssetContainer = class extends AbstractScene {
  ...
  mergeAnimationsTo(scene = EngineStore.LastCreatedScene, animatables, targetConverter = null) {
    ...
    const _targetConverter = targetConverter ? targetConverter : (target) => {
      ...
      const name6 = target.name.split(".").join("").split("_primitive")[0];
      switch (targetProperty) {
        case "position":
        case "rotationQuaternion":
          node = scene.getTransformNodeByName(target.name) || scene.getTransformNodeByName(name6);
          break;
        case "influence":
          node = scene.getMorphTargetByName(target.name) || scene.getMorphTargetByName(name6);
          break;
        default:
          node = scene.getNodeByName(target.name) || scene.getNodeByName(name6);
      }
      return node;
    };
...
var Sound = class _Sound {
   ...
   */
  constructor(name6, urlOrArrayBuffer, scene, readyToPlayCallback = null, options) {
    var _a;
    this.autoplay = false;
    this._loop = false;
...
var Skeleton = class _Skeleton {
  ...
  toString(fullDetails) {
    ...
    if (fullDetails) {
      ret += ", Ranges: {";
      let first = true;
      for (const name6 in this._ranges) {
        if (first) {
          ret += ", ";
          first = false;
        }
   ...
   */
  getBoneIndexByName(name6) {
    for (let boneIndex = 0, cache = this.bones.length; boneIndex < cache; boneIndex++) {
      if (this.bones[boneIndex].name === name6) {
        return boneIndex;
      }
    }
    return -1;
   ...
   */
  createAnimationRange(name6, from, to) {
    if (!this._ranges[name6]) {
      this._ranges[name6] = new AnimationRange(name6, from, to);
      for (let i = 0, nBones = this.bones.length; i < nBones; i++) {
        if (this.bones[i].animations[0]) {
          this.bones[i].animations[0].createRange(name6, from, to);
        }
      }
    }
  }
   ...
   */
  deleteAnimationRange(name6, deleteFrames = true) {
    for (let i = 0, nBones = this.bones.length; i < nBones; i++) {
      if (this.bones[i].animations[0]) {
        this.bones[i].animations[0].deleteRange(name6, deleteFrames);
      }
    }
    this._ranges[name6] = null;
  }
  /**
   ...
   */
  getAnimationRange(name6) {
    return this._ranges[name6] || null;
  }
  /**
  ...
  getAnimationRanges() {
    ...
    let name6;
    for (name6 in this._ranges) {
      animationRanges.push(this._ranges[name6]);
    }
   ...
   */
  copyAnimationRange(source, name6, rescaleAsRequired = false) {
    if (this._ranges[name6] || !source.getAnimationRange(name6)) {
      return false;
    }
    let ret = true;
    ...
    for (i = 0, nBones = this.bones.length; i < nBones; i++) {
      ...
      if (sourceBone) {
        ret = ret && this.bones[i].copyAnimationRange(sourceBone, name6, frameOffset, rescaleAsRequired, skelDimensionsRatio);
      } else {
        Logger.Warn("copyAnimationRange: not same rig, missing source bone " + boneName);
        ret = false;
      }
    }
    const range = source.getAnimationRange(name6);
    if (range) {
      this._ranges[name6] = new AnimationRange(name6, range.from + frameOffset, range.to + frameOffset);
    }
    return ret;
  }
  /**
   ...
   */
  beginAnimation(name6, loop, speedRatio, onAnimationEnd) {
    const range = this.getAnimationRange(name6);
    if (!range) {
      return null;
    }
   ...
   */
  clone(name6, id) {
    const result = new _Skeleton(name6, id || name6, this._scene);
  ...
  serialize() {
    ...
    for (let index = 0; index < this.bones.length; index++) {
      ...
      for (const name6 in this._ranges) {
        const source = this._ranges[name6];
        if (!source) {
          continue;
        }
        const range = {};
...
Node.AddNodeConstructor("TargetCamera", (name6, scene) => {
  return () => new TargetCamera(name6, Vector3.Zero(), scene);
});
var TargetCamera = class _TargetCamera extends Camera {
   ...
   */
  constructor(name6, position, scene, setActiveOnSceneIfNoneActive = true) {
    super(name6, position, scene, setActiveOnSceneIfNoneActive);
  ...
  createRigCamera(name6, cameraIndex) {
    if (this.cameraRigMode !== Camera.RIG_MODE_NONE) {
      const rigCamera = new _TargetCamera(name6, this.position.clone(), this.getScene());
...
Node.AddNodeConstructor("Light_Type_1", (name6, scene) => {
  return () => new DirectionalLight(name6, Vector3.Zero(), scene);
});
var DirectionalLight = class extends ShadowLight {
   ...
   */
  constructor(name6, direction, scene) {
    super(name6, scene);
...
Node.AddNodeConstructor("Light_Type_2", (name6, scene) => {
  return () => new SpotLight(name6, Vector3.Zero(), Vector3.Zero(), 0, 0, scene);
});
var SpotLight = class _SpotLight extends ShadowLight {
   ...
   */
  constructor(name6, position, direction, angle, exponent, scene) {
    super(name6, scene);
    this._innerAngle = 0;
...
var MorphTarget = class _MorphTarget {
   ...
   */
  constructor(name6, influence = 0, scene = null) {
    this.name = name6;
    this.animations = [];
   ...
   * @returns a new MorphTarget
   */
  static FromMesh(mesh, name6, influence) {
    if (!name6) {
      name6 = mesh.name;
    }
    const result = new _MorphTarget(name6, influence, mesh.getScene());
...
var name3 = "gaussianSplattingVertexDeclaration";
var shader3 = `uniform mat4 world;uniform mat4 view;uniform mat4 projection;
`;
...
var name4 = "gaussianSplattingUboDeclaration";
...
var name5 = "gaussianSplattingVertexShader";
var shader5 = `#include<__decl__gaussianSplattingVertex>
...
var DracoCompression = class _DracoCompression {
   ...
   */
  async decodeMeshToGeometryAsync(name6, scene, data, attributes) {
    const meshData = await this._decodeMeshAsync(data, attributes);
    const geometry = new Geometry(name6, scene);
    if (meshData.indices) {
  ...
  /** @internal */
  async _decodeMeshToGeometryForGltfAsync(name6, scene, data, attributes, gltfNormalizedOverride) {
...
var GaussianSplattingMesh = class _GaussianSplattingMesh extends Mesh {
   ...
   */
  constructor(name6, url = null, scene = null) {
    super(name6, scene);
    this._vertexCount = 0;
    this._worker = null;
  ...
  static ConvertPLYToSplat(data) {
    ...
    for (const prop of filtered) {
      const [, type, name6] = prop.split(" ");
      properties.push({ name: name6, type, offset: rowOffset });
      if (offsets[type]) {
...
var FlowGraphConnection = class {
  constructor(name6, _connectionType, _ownerBlock) {
...
var FlowGraphDataConnection = class extends FlowGraphConnection {
   ...
   * @param richType
   */
  constructor(name6, connectionType, ownerBlock, richType) {
    super(name6, connectionType, ownerBlock);
...
var FlowGraphBlock = class {
   ...
   */
  registerDataInput(name6, richType) {
    const input = new FlowGraphDataConnection(name6, FlowGraphConnectionType.Input, this, richType);
   ...
   * @returns the created connection
   */
  registerDataOutput(name6, richType) {
    const output = new FlowGraphDataConnection(name6, FlowGraphConnectionType.Output, this, richType);
   ...
   */
  getDataInput(name6) {
    return this.dataInputs.find((i) => i.name === name6);
  }
  /**
   ...
   */
  getDataOutput(name6) {
    return this.dataOutputs.find((i) => i.name === name6);
  }
  /**
...
var FlowGraphExecutionBlock = class extends FlowGraphBlock {
  ...
  _registerSignalInput(name6) {
    const input = new FlowGraphSignalConnection(name6, FlowGraphConnectionType.Input, this);
    this.signalInputs.push(input);
    return input;
  }
  _registerSignalOutput(name6) {
    const output = new FlowGraphSignalConnection(name6, FlowGraphConnectionType.Output, this);
   ...
   */
  getSignalInput(name6) {
    return this.signalInputs.find((input) => input.name === name6);
  }
  /**
   ...
   * @returns if the input exists, the input. Otherwise, undefined.
   */
  getSignalOutput(name6) {
    return this.signalOutputs.find((output) => output.name === name6);
  }
  /**
   * Serializes this block
...
var FlowGraphContext = class {
   ...
   */
  hasVariable(name6) {
    return name6 in this._userVariables;
  }
  /**
   ...
   */
  setVariable(name6, value) {
    this._userVariables[name6] = value;
  }
  /**
   ...
   */
  getVariable(name6) {
    return this._userVariables[name6];
  }
  /**
  ...
  _getUniqueIdPrefixedName(obj, name6) {
    return `${obj.uniqueId}_${name6}`;
  }
  /**
   ...
   * @param value
   */
  _setExecutionVariable(block, name6, value) {
    this._executionVariables[this._getUniqueIdPrefixedName(block, name6)] = value;
  }
  /**
   ...
   * @returns
   */
  _getExecutionVariable(block, name6, defaultValue) {
    if (this._hasExecutionVariable(block, name6)) {
      return this._executionVariables[this._getUniqueIdPrefixedName(block, name6)];
    } else {
      return defaultValue;
    }
  }
   ...
   * @param name
   */
  _deleteExecutionVariable(block, name6) {
    delete this._executionVariables[this._getUniqueIdPrefixedName(block, name6)];
  }
  /**
   ...
   * @returns
   */
  _hasExecutionVariable(block, name6) {
    return this._getUniqueIdPrefixedName(block, name6) in this._executionVariables;
  }
  /**
...
//# sourceMappingURL=chunk-OTC5K42J.js.map
